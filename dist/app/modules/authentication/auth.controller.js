...
exports.loginUser = (0, utils_1.catchAsync)(async (req, res, next) => {
    ...
    if (!user) {
        await auth_repository_1.userRepository.create({ email, verificationCode: hashedCode });
    }
    else {
        await auth_repository_1.userRepository.updateById(user._id, { verificationCode: hashedCode });
    }
